...
const auditTimeline = computed(() => {
  const refund = detailData.value;
  if (!refund?.auditLogs?.length) return [];
  return refund.auditLogs.map(item => ({
    timestamp: item.createTime,
    title: item.actionText || item.actionCode,
    content:
      item.operatorDisplayName && item.remark
        ? `${item.operatorDisplayName} - ${item.remark}`
        : item.operatorDisplayName || item.remark || item.operatorName || "-",
    type: (item.actionCode === "REJECTED"
      ? "danger"
      : item.actionCode === "REFUNDED"
        ? "success"
        : item.actionCode === "APPROVED"
          ? "primary"
          : "warning") as "primary" | "success" | "warning" | "danger" | "info"
  }));
});
...
const loadRefundStatistics = async () => {
  ...
  try {
    statistics.value = await refundApi.getRefundStatistics();
  } finally {
...
const submitAudit = async () => {
  ...
  try {
    ...
    await Promise.all([loadRefundStatistics(), loadRefundList()]);
  } finally {
  ...
  <div class="app-container">
    <div class="stats-grid">
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.totalRefunds"
          :value="statistics?.totalRefunds || 0"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.pendingRefunds"
          :value="statistics?.pendingRefunds || 0"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.refundedCount"
          :value="statistics?.refundedCount || 0"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.rejectedRefunds"
          :value="statistics?.rejectedRefunds || 0"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.totalRefundAmount"
          :value="Number(statistics?.totalRefundAmount || 0)"
          :precision="2"
          prefix="￥"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.refundedAmount"
          :value="Number(statistics?.refundedAmount || 0)"
          :precision="2"
          prefix="￥"
        />
      </el-card>
      <el-card v-loading="statsLoading" shadow="never">
        <el-statistic
          :title="texts.pendingAmount"
          :value="Number(statistics?.pendingAmount || 0)"
          :precision="2"
          prefix="￥"
        />
      </el-card>
    ...
    >
      ...
      <div class="timeline-block">
        <div class="timeline-title">{{ texts.auditTrace }}</div>
        <el-timeline v-if="auditTimeline.length">
          ...
          >
            <div class="timeline-content">
              ...
              <div class="timeline-sub">{{ item.content || "-" }}</div>
            </div>
          </el-timeline-item>
        </el-timeline>
        <el-empty v-else :description="texts.noTrace" />
      </div>
    </el-dialog>
